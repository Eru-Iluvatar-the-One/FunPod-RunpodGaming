...
} catch { W "Pod may already be running." "Yellow" }

# ── Poll until RUNNING + get SSH details ────────────────────────────
W "Waiting for pod..."
$ip = ""; $port = 0; $tries = 0
do {
    Start-Sleep 5
    $tries++
    if ($tries -gt 48) { W "Timed out after 4 min. Check RunPod dashboard." "Red"; Read-Host; exit 1 }
...
W "Pod at ${ip}:${port} — giving it 12s to finish booting..."
Start-Sleep 12

# ── Build pod setup script (bash) ───────────────────────────────────
...
# Install Parsec binary if missing
...
# Fresh auth if no saved session
...
# Push updated config to B2 for next session
b2 upload-file FunFun "`$CFG" parsec/config.cfg 2>/dev/null \
|| b2 file upload FunFun "`$CFG" parsec/config.cfg 2>/dev/null \
|| true
...
W "Setting up Parsec on pod (2-3 min first run)..."
...
Start-Sleep 3
W "Connect to your pod under 'My Computers' in Parsec." "Green"
Start-Sleep 5
